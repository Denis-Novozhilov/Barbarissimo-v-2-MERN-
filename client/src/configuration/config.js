export const STORAGE_AUTHENTICATION = 'userData';
export const STORAGE_GAME_LANGUAGE_SETTINGS = 'userGameLanguageSettings';


// api/auth
export const REGISTRATION_URL = 'api/auth/register';
export const LOGIN_URL = 'api/auth/login';
export const PING_URL = 'api/auth/ping';

// api/game
export const CREATE_PHRASE_URL = 'api/game/createPhraseTest';
export const CREATE_COMMON_PHRASE_URL = ' api/game/createCommonPhrase';
export const GET_ALL_COMMON_PHRASES_URL = 'api/game/allCommonPhrases';

// api/admin
export const CREATE_USER_PHRASE = 'api/admin/test-create-user-phrase';